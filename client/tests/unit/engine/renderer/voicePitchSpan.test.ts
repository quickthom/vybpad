...
describe('computeVoicePitchSpan — UI-R2-W5.1', () => {
  ...
  it('expands a narrow span to the 12-semitone floor and sets minSpanApplied', () => {
    const result = computeVoicePitchSpan(
      songWithVoices({
        ...emptyMeasure('narrow'),
        notes: [
  ...
  it('keeps exact bounds when natural span is already >= 12 semitones', () => {
    const result = computeVoicePitchSpan(
      songWithVoices({
        ...emptyMeasure('wide'),
        notes: [
